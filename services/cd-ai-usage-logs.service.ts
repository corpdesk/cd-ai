...
export class CdAiUsageLogsService {
   ...
   * @param q
   */
  async getCdAiUsageLogs(req, res, q?: IQuery): Promise<any> {
    if (q === null) {
      q = this.b.getQuery(req);
    }
    // const serviceInput = siGet(q,this)
    this.serviceModel = new CdAiUsageLogsModel();
    // const serviceInput: IServiceInput<CdAiUsageLogsModel> = this.b.siGet(q, this);
    const serviceInput: IServiceInput<any> = this.b.serviceInputGet(
      q,
