...
export class CdAiTypeService {
  ...
  async create(req: any, res: any) {
    this.logger.logInfo("CdAiTypecreate::validateCreate()/01");
    ...
    if (await this.validateCreate(req, res)) {
      ...
      this.logger.logInfo("CdAiTypecreate::validateCreate()/02");
      const r = await this.b.respond(req, res);
    }
  }
  ...
  async getCdAiType(req, res, q?: IQuery): Promise<any> {
    ...
    this.logger.logInfo("CdAiTypeService::getCdAiTypef:", q);
    const serviceInput = this.b.siGet(
      q,
      "CdAiTypeService::getCdAiTypef",
      CdAiTypeModel,
    );
    try {
